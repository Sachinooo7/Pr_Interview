...
export const generateQuestion=async (req,res)=>{
   try{
    ...
    `;
    if(!userPrompt.trim()){
        return res.status(400).json({message:"Insufficient data to generate questions"});
...
`
 ...
 export const getMyInterviews=async (req,res)=>{
    try{
        ...
        .sort({createdAt:-1})
        .select("role experience mode finalScore status Created");
 ...
 export const getInterviewReport=async (req,res)=>{
    try{
        ...
        console.log("failed to find report",error);
    }
 }
